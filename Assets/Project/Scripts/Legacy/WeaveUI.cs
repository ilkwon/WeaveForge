...
public class WeaveUI : MonoBehaviour
{
  ...
  public void OnSaveButton()
  {
    ...
    // 크기가 바뀐 경우만 Resize
    if (x != data.colCount || y != data.rowCount)
  ...
  private void OnLoadButton(string weaveName)
  {
    ...
    texboxUnitWidth.text = data.colCount.ToString();
    textboxUnitHeight.text = data.rowCount.ToString();

    colorStripWarp.Setup(data.colCount, true);
  ...
  {       
    ...
    render.material.mainTextureScale = new Vector2(data.colCount*factor, data.rowCount*factor);
    ...
    TabKeyNextCursorUnitSize();
      ...
      if (currentData != null)
      {
        ...
        sphereRenderer.material.mainTextureScale = 
          new Vector2(currentData.colCount * factor, currentData.rowCount * factor);
